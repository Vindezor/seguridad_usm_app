...
class HomeController extends ChangeNotifier {
  ...
  Future<void> emergencia(BuildContext context) async {
    ...
    try {
      ...
      final response = await emergenciaService.emergencia(
          name!, email!, '${cords.latitude}, ${cords.longitude}', phone!);

      Navigator.of(context).pop();
      if (response!.status == "SUCCESS") {
        globalAlert(context, msg: response.msg!, title: "Importante",
            closeOnPressed: () {
          Navigator.of(context).pop();
        });
      } else {
      ...
      log('$e');
      globalAlert(context, msg: "Error al enviar correo", title: "Importante",
          closeOnPressed: () {
        Navigator.of(context).pop();
      });
    }
